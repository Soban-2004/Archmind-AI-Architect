...
export function SimulationPanel({ projectId, versionId, state, result, onResult, onExit }: Props) {
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <p className="px-4 pt-2.5 text-[11px] leading-snug text-slate-400 italic">
        Heuristic capacity model based on declared per-component assumptions — not a real load test.
      </p>

      <div className="space-y-3.5 border-b border-slate-100 px-4 py-3.5">
        <div>
          <p className="mb-1.5 flex items-center gap-1 text-xs font-medium text-slate-600">
            ...
            {MULTIPLIER_PRESETS.map((m) => (
              <button
                ...
                className={`rounded-lg px-2.5 py-1 text-xs font-semibold transition-colors ${
                  multiplier === m ? "bg-brand-600 text-white shadow-sm" : "bg-slate-100 text-slate-500 hover:bg-slate-200"
                }`}
              >
                {m}×
              </button>
        ...
        <div>
          ...
          <div className="max-h-28 space-y-1 overflow-y-auto rounded-lg border border-slate-100 bg-slate-50 p-2">
            {state.nodes.map((n) => (
              <label key={n.id} className="flex cursor-pointer items-center gap-1.5 text-xs text-slate-600">
                <input
                  type="checkbox"
        ...
        {result && (
          <>
            <div className="flex items-center justify-between">
              <p className="text-xs italic text-slate-500">Scenario: {result.scenario}</p>
              <button onClick={() => onResult(null)} className="text-[10px] text-slate-400 underline">
                clear
              </button>
            </div>
            {result.findings.length === 0 ? (
              <p className="mt-2 text-xs font-medium text-green-600">No bottlenecks under this scenario.</p>
            ) : (
              <div className="mt-2 space-y-1.5">
                {result.findings.map((f) => (
                  <div key={`${f.order}-${f.node_id}`} className="rounded-lg border border-red-200 bg-red-50 px-2.5 py-1.5">
                    <p className="text-xs font-semibold text-red-700">
                      #{f.order} {f.node_name}
                    </p>
                    <p className="text-[11px] leading-snug text-red-600">{f.message}</p>
                  </div>
                ))}
              </div>
            )}
...
function statusColor(status: LoadStatus): string {
  if (status === "overloaded") return "font-semibold text-red-600";
