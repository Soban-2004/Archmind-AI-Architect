...
const KIND_STYLE: Record<NodeKind, { bg: string; border: string; icon: string; label: string; Icon: typeof Server }> = {
  service: { bg: "bg-blue-50", border: "border-blue-300", icon: "text-blue-500", label: "Service", Icon: Server },
  database: { bg: "bg-emerald-50", border: "border-emerald-300", icon: "text-emerald-500", label: "Database", Icon: Database },
  queue: { bg: "bg-purple-50", border: "border-purple-300", icon: "text-purple-500", label: "Queue", Icon: Layers },
  external_dependency: { bg: "bg-orange-50", border: "border-orange-300", icon: "text-orange-500", label: "External", Icon: Globe },
  infra_node: { bg: "bg-slate-100", border: "border-slate-300", icon: "text-slate-500", label: "Infra", Icon: Cloud },
};

const DIFF_RING: Record<DiffStatus, string> = {
  added: "ring-2 ring-green-500 ring-offset-2",
...
const SIM_RING: Record<LoadStatus, string> = {
  ok: "ring-2 ring-green-400 ring-offset-2",
  warning: "ring-2 ring-amber-500 ring-offset-2",
  overloaded: "ring-4 ring-red-500 ring-offset-2 animate-pulse",
...
export function ArchNodeCard({ data, selected }: NodeProps) {
  ...
  return (
    <div
      className={`relative w-[200px] rounded-xl border ${style.border} ${style.bg} px-3.5 py-3 shadow-[0_1px_2px_rgba(15,23,42,0.06)] transition-shadow ${ring} ${
        selected ? "shadow-md" : ""
      }`}
    >
      {badge && (
      ...
      <div className="flex items-center gap-2">
        <div className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-white/70 ${style.icon}`}>
          <Icon size={15} strokeWidth={2.25} />
        </div>
        <div className="min-w-0">
          <div className="truncate text-[13px] font-semibold text-slate-800">{node.name}</div>
