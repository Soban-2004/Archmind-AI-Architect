...
async def _load_state(project_id: UUID, version_id: UUID, owner_token: Optional[str]) -> ArchitectureState:
    version = await repo.get_version(version_id, owner_token)
...
async def get_scorecard(project_id: UUID, version_id: UUID, x_guest_token: Optional[str] = Header(default=None)):
    state = await _load_state(project_id, version_id, x_guest_token)
    return score_architecture(state)


@router.post("/{project_id}/versions/{version_id}/scorecard/ask")
@limiter.limit(LLM_LIMIT)
async def ask_scorecard(request: Request, response: Response, project_id: UUID, version_id: UUID, body: dict, x_guest_token: Optional[str] = Header(default=None)):
