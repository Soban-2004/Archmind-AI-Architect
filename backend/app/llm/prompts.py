...
- node_type="external_dependency": {"type": one of "third_party_api" | "payment" | "market_data" | "auth_provider" | "storage" | "notification_provider" | "analytics" (use "third_party_api" for a generic external API — NOT "api"; "notification_provider" for a service that sends email/SMS/push on the system's behalf, e.g. Twilio/SendGrid — not "third_party_api"; "analytics" for a product-analytics/telemetry destination the system sends events to, e.g. Segment/Mixpanel), "criticality"?: "hard" | "soft", "rationale"?: string (see below)}
